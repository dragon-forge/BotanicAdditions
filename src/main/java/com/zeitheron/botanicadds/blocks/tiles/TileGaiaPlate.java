...
import com.zeitheron.botanicadds.net.PacketGaiaPlateEffect;
...
public class TileGaiaPlate extends TileMod implements ISparkAttachable, ITickable, IItemHandler
{
	...
	RecipeGaiaPlate areItemsValid(List<EntityItem> items)
	{
		if(items.size() != 3)
			return null;
		for(RecipeGaiaPlate rgp : GaiaPlateRecipes.gaiaRecipes)
			if(rgp.matches(this))
				return rgp;
		return null;
	}
