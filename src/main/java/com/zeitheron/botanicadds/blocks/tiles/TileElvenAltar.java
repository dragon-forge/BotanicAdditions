package com.zeitheron.botanicadds.blocks.tiles;
...
public class TileElvenAltar extends TileSimpleInventory implements IManaReceiver, ITickable, ISparkAttachable
{
	...
	public void update()
	{
		...
		int avaliable = getAvailableSpaceForMana();
		if(avaliable > 0)
		{
			ISparkEntity spark = getAttachedSpark();
			if(spark != null)
				for(ISparkEntity otherSpark : SparkHelper.getSparksAround(world, pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5))
				{
					if(spark == otherSpark)
						continue;
					if(otherSpark.getAttachedTile() instanceof IManaPool)
	...
	public void onWanded(EntityPlayer player, ItemStack wand)
	{
		...
		if(manaToGet > 0 && mana >= manaToGet)
		{
			...
			if(livingrock != null)
			{
				...
				currentRecipe = null;
				world.addBlockEvent(getPos(), getBlockType(), SET_COOLDOWN_EVENT, 60);
